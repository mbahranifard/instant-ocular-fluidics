function[FEyej,Pf,fig1,fig2,fig8,slope] = DirectComp (stepP,stepQ,stepPa,steptmin,actoff,Cc,FSys,Yr,B,PrC,KeepStepsUp,gracep,PLOT)
...
plot (t*60,P,'color','b');
hold on 
plot(t*60, Pnofilt,'color','r');
plot(t(actoff(target))*60,Pnofilt(actoff(target)),'marker', 'O','color','g');
ylabel ('P (mmHg)'); xlabel('t (s)');
...
subplot(3,4,i);hold all;box on;grid on 
plot (t(1:actoff(target)+gracep)*60,dP(1:actoff(target)+gracep)/dt,'color','b');
hold on 
plot(t(1:actoff(target)+gracep)*60,dPnofilt(1:actoff(target)+gracep)/dt,'color','r');
plot(t(actoff(target))*60,dPnofilt(actoff(target))/dt,'marker', 'O','color','g');
xlabel('t (s)'); ylabel('dP/dt (mmHg/min)');
title(['Step ' num2str(target)]);
end
%% second derivative
...
FEye = (Q - (exp(Yr)*(P/PrC).^B) .*P - derP * FSys)./ derP;
% close all
if PLOT
fig8 = figure(108);
...
plot(t*60,FEye);
hold on
plot(t(actoff(target))*60,FEye(actoff(target)),'marker', 'O','color','r');
plot([t(actoff(target)+1:actoff(target)+6)*60],FEye(actoff(target)+1:actoff(target)+6),'color','g','linewidth',1.2)
ylabel('\phi (nl/min)'); xlabel('t (s)');
title(['Step ' num2str(target)]);
xlim([t(130)*60 t(180)*60]);
end
